...
| Action | Binding |
|---|---|
| Create or resume a session | `rr <name>` |
| List sessions | `rl` |
...
| Zoom pane | `prefix + z` |
| Show all prefix bindings | `prefix + ?` |
...
There are 278 effective bindings across four tables. Refresh the snapshot with an isolated server:
...
rmux -L "$socket" list-keys -T root
```

## Prefix table

90 bindings. This block is generated from the effective RMUX server.


<!-- BEGIN GENERATED prefix -->
```text
...
bind-key    -T prefix T       if-shell -F "#{mouse}" "set -g mouse off ; display-message \"mouse OFF: native terminal selection enabled\"" "set -g mouse on ; display-message \"mouse ON: pane mouse mode enabled\""
...
## Vi copy-mode table (active)

89 bindings. This block is generated from the effective RMUX server.


<!-- BEGIN GENERATED copy-mode-vi -->
```text
...
## Emacs-style copy-mode table (retained, inactive)

75 bindings. This block is generated from the effective RMUX server.


<!-- BEGIN GENERATED copy-mode -->
```text
...
bind-key -T copy-mode C-l               send-keys -X recentre-top-bottom
...
```
<!-- END GENERATED copy-mode -->

## Root mouse table

24 bindings. This block is generated from the effective RMUX server.
...
See [RMUX](RMUX.md) for session lifecycle, configuration, and troubleshooting.
