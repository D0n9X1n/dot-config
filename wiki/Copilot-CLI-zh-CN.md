...
`settings.json` 也设置了 `defaultPermissionMode: "allow-all"`，用于不经过 alias 启动的新交互会话。Alias 还覆盖恢复会话和 `-p` 调用。这只改变权限，不会启用 autopilot。GPT-6 Astra、长 context 和 high effort 为默认值；`gg` 也会在启动时固定这些设置。这些默认值与 [Claude Code](Claude-Code-zh-CN.md) 独立；后者以 Opus 5.5 和 `xhigh` 启动。单次覆盖请用 `copilot --effort <level>`；`gg` 的参数是标题，不是 CLI flags。
