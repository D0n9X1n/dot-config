...
The active `config/rmux/rmux.conf` retains the `C-q` prefix, Apollo top status bar, one-based indices, current-directory splits and windows, vim pane navigation and resizing, vi copy mode, truecolor handling, persistent titles, and explicit `prefix + Tab` behavior.
