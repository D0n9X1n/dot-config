...
| 列出会话 | `rl` |
| 删除会话 | `rd <名称>` |
| 分离但不结束会话 | `prefix + d`、`exit`、`logout`、空提示符 Ctrl+D，或关闭标签页 |
| 重载配置 | `prefix + r` |
| 在当前目录新建窗口 | `prefix + c` |
| 向右 / 向下分割 | `prefix + \|` / `prefix + -` |
| 在窗格间移动 | `prefix + h/j/k/l` |
| 连续调整窗格大小 | `prefix + H/J/K/L` |
| 返回上一个窗口 | `prefix + Tab` |
| 切换鼠标 / 原生选择 | `prefix + T` |
| 进入复制模式 | `prefix + v` 或 `prefix + [` |
| 缩放窗格 | `prefix + z` |
| 显示全部 prefix 按键 | `prefix + ?` |
...
四个表共包含 278 个生效绑定。可使用独立 server 刷新快照：
...
共 90 个绑定。以下内容由生效中的 RMUX server 生成。
...
共 89 个绑定。以下内容由生效中的 RMUX server 生成。
...
## Emacs 风格复制模式表（保留但未启用）

共 75 个绑定。以下内容由生效中的 RMUX server 生成。
...
共 24 个绑定。以下内容由生效中的 RMUX server 生成。
...
会话生命周期、配置和故障排查请参阅 [RMUX](RMUX-zh-CN.md)。
